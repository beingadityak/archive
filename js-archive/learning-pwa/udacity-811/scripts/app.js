...
(function() {
  ...
  app.updateForecastCard = function(data) {
    ...
    // verify data is newer than we already have. If not, bail
  ...
  app.getForecast = function(key, label) {
    ...
    if('caches' in window)                          // request from cache
    {
        caches.match(url).then(function(response){
          if(response){
            response.json().then(function(json){
              /*update if XHR is still pending else XHR has returned with latest data
              */
    ...
    app.hasRequestPending = true;       // flag indicating n/w req pending & use cache data
    ...
    window.localforage.getItem('selectedCities',function(err,cityList){
      if(cityList)
      {
        ...
        app.selectedCities.forEach(function(city){
          app.getForecast(city.key,city.value);
        });
      }
      else {
